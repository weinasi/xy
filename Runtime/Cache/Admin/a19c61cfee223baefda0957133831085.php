...
<script>
	...
	<div class="cf">
		<div class="fl">
			<a class="btn" href="<?php echo U('Article/activityAdd');?>">添加</a>
		...
		<div class="search-form fr cf">
			<div class="sleft">
				...
				<input type="text" name="title" class="search-input" value="<?php echo I('title');?>" placeholder="请输入活动名称">
    ...
    <tbody>
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="tr_css">
			...
			<td><img style="height: 40px;width: 40px;" src="<?php echo ($vo["fmt"]); ?>" alt=""></td>
			<td>
				<?php foreach ($vo['xcts'] as $xct){ ?>
				<img style="height: 30px;width: 30px;" src="/xy<?php echo $xct; ?>" alt=""> <?php  }?>
			</td>
			<td><?php echo ($vo["mansl"]); ?></td>
			<td><?php echo ($vo["bmmansl"]); ?></td>
			<td><?php echo ($vo["womensl"]); ?></td>
			<td><?php echo ($vo["bmwomensl"]); ?></td>
			<td><?php echo ($vo["qtsl"]); ?></td>
			<td><?php echo ($vo["bmqtsl"]); ?></td>
			<td><?php echo ($vo["llcs"]); ?></td>
