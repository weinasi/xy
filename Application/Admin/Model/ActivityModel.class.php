...
//        var_dump($data);
    	/* 检查文档类型是否符合要求 */
    	$res = $this->checkActivityType(I('post.'));
    	if(!$res['status']){
    		$this->error = $res['info'];
    		return false;
    	}
    ...
    public function autoSave(){
        ...
        /* 检查文档类型是否符合要求 */
        $res = $this->checkActivityType( I('type'), I('pid') );
        if(!$res['status']){
        	$this->error = $res['info'];
        	return false;
        }
     ...
     * @author huajie <banhuajie@163.com>
     */
    public function checkActivityType($data){
        ...
        $hdbh = $data['hdbh'];//活动编号
        if(empty($hdbh)){
            return array('status'=>0, 'info'=>'活动编号不能为空');
        }

        $startAndEnd = $data['startAndEnd'];//活动开始时间与结束时间
        if(empty($startAndEnd)){
            return array('status'=>0, 'info'=>'活动开始时间与结束时间不能为空');
        }


//        $rqs = $data['rqs'];//日期始
//        if(empty($rqs)){
//            return array('status'=>0, 'info'=>'开始日期不能为空');
//        }
//
//        $rqz = $data['rqz'];//日期止
//        if(empty($rqz)){
//            return array('status'=>0, 'info'=>'结束日期不能为空');
//        }
        ...
        if(empty($latitude)){
            return array('status'=>0, 'info'=>'纬度不能为空');
        }

        $name = $data['name'];//位置名
        if(empty($name)){
            return array('status'=>0, 'info'=>'位置名不能为空');
        }

        $fmt = $data['fmt'];//封面图
        if(empty($fmt)){
            return array('status'=>0, 'info'=>'封面图不能为空');
        }

        $zt = $data['zt'];//状态
        if(empty($zt)){
            return array('status'=>0, 'info'=>'状态不能为空');
        }

        $zsl = $data['zsl'];//总数量
        if(empty($zsl)){
            return array('status'=>0, 'info'=>'总数量不能为空');
        }

        $bmjg = $data['bmjg'];//报名价格
        if(empty($bmjg)){
            return array('status'=>0, 'info'=>'报名价格不能为空');
        }

        $xct = $data['xct'];//宣传图
        if(empty($xct)){
            return array('status'=>0, 'info'=>'宣传图不能为空');
        }

        $mansl = $data['mansl'];//男士数量
        if(empty($mansl)){
            return array('status'=>0, 'info'=>'男士数量不能为空');
        }

        $womensl = $data['womensl'];//女士数量
        if(empty($womensl)){
            return array('status'=>0, 'info'=>'女士数量不能为空');
        }
