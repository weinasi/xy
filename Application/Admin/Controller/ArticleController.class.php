...
class ArticleController extends AdminController {
     ...
     * @author huajie <banhuajie@163.com>
     */
    public function activity($status = null, $title = null){
        //获取左边菜单
        $this->getMenu();
        ...
        //只查询pid为0的文章
        $map['pid'] = 0;
        $list = $this->lists($Activity,[],'id desc');
        int_to_string($list);
        // 记录当前列表页的cookie
        Cookie('__forward__',$_SERVER['REQUEST_URI']);
        $this->assign('status', $status);
...
//        dump($list);
    ...
    public function activityEdit(){
        ...
        /*获取一条记录的详细数据*/
        $Activity = D('Activity');
        $activity = $Activity->find(($data['id'] = $id));
        if(!$activity){
...
//        dump($fmts);
        ...
        $xcts = explode(',',substr($activity['xct'],0,strlen($activity['xct'])-1));
//        dump($xcts);
        $this->assign('xcts', $xcts);
    ...
    public function setActivityStatus(){
        if($_GET['id']){
            $res = D('Activity')->deleteActivity([$_GET['id']]);
        }

        if($_POST['ids']){
            $res = D('Activity')->deleteActivity($_POST['ids']);
        }
    ...
    public function imgDelete(){
        ...
        $Activity   =   D('Activity');
        if(is_null($imageId)){
            $this->AjaxReturn(['errorCode'=>1,'msg'=>'imageId is null']);
        }
        $list = $this->lists($Activity,[],'id desc');
        $isStop = false;
        foreach ($list as $l){
            if($l['xct']){
                $fmts = explode(',',substr($l['xct'],0,strlen($l['xct'])-1));
                if(in_array($imageId,$fmts)){
                    $index = array_search($imageId,$fmts);
                    unset($fmts[$index]);
                    $activity['id'] = $l['id'];
                    $activity['xct'] = implode(',',$fmts).',';
                    (new ActivityModel())->save($activity);
                    $isStop = true;
                    break;
                }else{
                    continue;
        ...
        if($isStop){
            $this->AjaxReturn(['errorCode'=>0,'msg'=>'success']);
        }else{
            $this->AjaxReturn(['errorCode'=>2,'msg'=>'fail']);
        }
